...
import { createResume, generateResumePdf, getMyResume } from "../api/resume";
import { getProfile } from "../api/user";
import { motion } from "framer-motion";
import { FiDownload, FiFileText, FiCheck, FiLoader } from "react-icons/fi";
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Auto-fill resume base data from profile
        setHeadline(
          `${user.firstName} ${user.lastName} — ${user.currentEducation?.degree || user.currentEducation_degree || "Job Seeker"}`
        );
        setEducation(
          `${user.currentEducation?.degree || user.currentEducation_degree || ""}, ${user.currentEducation?.college || user.currentEducation_college || ""}`
        );
        ...
        try {
          ...
          if (existing) {
            ...
            // We could also pre-fill fields if we saved them, but for now we just show the PDF link
          }
        } catch (e) {
          // ignore if no resume
        }
